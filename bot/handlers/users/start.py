...
from states import ReedLesson

# from bot.data.config import CHANNEL_URL
# from bot.filters.chat_member import ChatMemberFilter
# from bot.filters.chat_type import ChatTypeFilter
# from bot.keyboards.start_bot_btn import to_ask_bot
# from bot.loader import dp,bot
# from bot.data.config import GROUP_URL, CHANNEL_URL, GROUP_ID

# from bot.data.recordlesson import RecordForLesson
# from bot.loader import dp,bot
# from bot.middlewares.weekend import WeekendMessageMiddleware
# from bot.states import ReedLesson
...
async def new_user(message: types.Message, state: FSMContext):
    try:
        ...
        # else:
        await bot.send_message(message.from_user.id,f"Здравствуйте, чем могу помочь?")
    except Exception:
...
async def start_command(call: types.CallbackQuery, state: FSMContext):
    if call.data == "ThisChannel":
        link_channel = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Канал", callback_data="EnterChannel")]])
